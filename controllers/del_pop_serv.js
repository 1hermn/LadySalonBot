...
const fs = require('fs')
const {checkPermForAdmin, checkPermForDirector, delete_PopService} = require('../tools/tools.js');
const del_pop_serv = new Scenes.WizardScene(
	'del_pop_serv',
	async (ctx) => {
        //вывести акции
		...
		ctx.reply("Отправьте номер популярной услуги, которую хотите удалить", admin_crontrol);
    ...
    async (ctx) => {
        if(ctx.message) {
            switch (ctx.message.text) {
                case "Да":
                    ctx.reply("Удалено")
                    delete_PopService(ctx.session.user.action_text)
                    break;
                case "Нет":
                    ctx.reply("Отменяю")
                    break;
